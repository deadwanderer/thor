...
_add_user :: proc(servername: string, username: string, password: string) -> (ok: NET_API_STATUS) {
	...
	ok = NetUserAdd(servername_w, level, &user_info, &parm_err)

	return
}

get_computer_name_and_account_sid :: proc(
	username: string,
) -> (
...
add_user_to_group :: proc(sid: ^SID, group: string) -> (ok: NET_API_STATUS) {
	...
	ok = NetLocalGroupAddMembers(nil, &group_name[0], 0, &group_member, 1)
	return
}
...
add_del_from_group :: proc(sid: ^SID, group: string) -> (ok: NET_API_STATUS) {
	...
	group_name := utf8_to_utf16(group, context.temp_allocator)
	ok = NetLocalGroupDelMembers(nil, &group_name[0], 0, &group_member, 1)
	return
}
...
add_user_profile :: proc(username: string) -> (ok: bool, profile_path: string) {
	...
	res2 := CreateProfile(sb, &username_w[0], &pszProfilePath[0], 257)
	if res2 != 0 {
		return false, ""
	}
...
delete_user_profile :: proc(username: string) -> (ok: bool) {
	...
	res2 := DeleteProfileW(sb, nil, nil)
	return bool(res2)
}
...
delete_user :: proc(servername: string, username: string) -> (ok: bool) {
	...
	*/
	...
	res := NetUserDel(servername_w, &username_w[0])
	if res != .Success {
		return false
	}
	return true
