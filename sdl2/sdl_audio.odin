...
AUDIO_ISFLOAT :: #force_inline proc "c" (x: AudioFormat) -> bool {return(
		(x & AUDIO_MASK_DATATYPE) ==
		AUDIO_MASK_DATATYPE \
	)}
AUDIO_ISBIGENDIAN :: #force_inline proc "c" (x: AudioFormat) -> bool {return(
		(x & AUDIO_MASK_ENDIAN) ==
		AUDIO_MASK_ENDIAN \
	)}
AUDIO_ISSIGNED :: #force_inline proc "c" (x: AudioFormat) -> bool {return(
		(x & AUDIO_MASK_SIGNED) ==
		AUDIO_MASK_SIGNED \
	)}
AUDIO_ISINT :: #force_inline proc "c" (x: AudioFormat) -> bool {return !AUDIO_ISFLOAT(x)}
AUDIO_ISLITTLEENDIAN :: #force_inline proc "c" (x: AudioFormat) -> bool {return(
		!AUDIO_ISBIGENDIAN(x) \
	)}
...
foreign lib {
	...
	OpenAudioDevice :: proc(device: cstring, iscapture: bool, desired: ^AudioSpec, obtained: ^AudioSpec, allowed_changes: bool) -> AudioDeviceID ---
}
...
@(default_calling_convention = "c", link_prefix = "SDL_")
foreign lib {
	BuildAudioCVT :: proc(cvt: ^AudioCVT, src_format: AudioFormat, src_channels: u8, src_rate: c.int, dst_format: AudioFormat, dst_channels: u8, dst_rate: c.int) -> c.int ---

	ConvertAudio :: proc(cvt: ^AudioCVT) -> c.int ---

	NewAudioStream :: proc(src_format: AudioFormat, src_channels: u8, src_rate: c.int, dst_format: AudioFormat, dst_channels: u8, dst_rate: c.int) -> ^AudioStream ---
